...
End-to-end sandbox for the `orchestrate` skill in [JeongJaeSoon/agent-skills](https://github.com/JeongJaeSoon/agent-skills): a tiny calculator, numbered migrations and role-named CI jobs (`lint`, `unit`) behind branch protection (required checks; branches need not be up to date). Programs run here are throwaway.
